...
export function RankBadge({ score, unranked }: { score?: Score; unranked?: boolean }) {
  ...
  if (!score) return <Badge variant="outline" className="text-muted-foreground">unscored</Badge>;
  const tone = score.rank >= 50 ? 'default' : score.rank >= 10 ? 'secondary' : 'outline';
  return <Badge variant={tone} title={score.hops !== undefined ? `${score.hops} hops` : undefined}>rank {score.rank}</Badge>;
}
