  ...
  return (
    <Layout>
      ...
      <article className="prose-sm max-w-3xl text-base">
        ...
        <P><strong>Canvassers</strong> fill the list. Add an item, tag a profile, from here or from any Nostr app that speaks decentralized lists. You do not need to know a campaign exists to be paid by one; if your entry is good, the arbiter finds it.</P>
        <P><strong>Arbiters</strong> judge and pay. They hold the escrow, look at each entry, and pay the canvasser directly over Lightning. Their judgement is public: every acceptance is a signed event, and every payment has a receipt. Patrons often arbitrate their own campaigns. Anyone can announce themselves as an arbiter.</P>

        <H>How it works</H>
        <P>Canvasstr is a client for <A href="https://catallax.network">Catallax</A>, an open protocol for contract work on Nostr with three parties: a patron who wants work done, a free agent who does it, and an arbiter who holds the money and settles. Canvasstr narrows that to one kind of work, curation, and stretches one task to many workers: a campaign is a Catallax task with no fixed worker, and every accepted entry gets its own conclusion and its own payment. The wire format is documented in the repository; there are no new event kinds, only a few extra tags.</P>
        <P>Payments are real Lightning payments from the arbiter's wallet to the canvasser's Lightning address, made as zaps so the receipt lands on the relays. The board flips an entry to "paid" the moment the receipt appears. Nothing here custodies your money except the arbiter you chose.</P>
